...
buckets_string <- strsplit(args[8],",")[[1]]

buckets <- c()
i = 0
for(i in 1:length(buckets_string))
{
  buckets[i] = as.numeric(buckets_string[i])
}
...
print("Bucketing overhead is (divide to initial overhead to get percentage): ")
print(overhead/(N))